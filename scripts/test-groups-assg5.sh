...
BASE_DIR=${1:-"datasets/student-submissions/assg5"}
ORACLE_DIR=${2:-"oracles/student-submissions/assg5"}
...
for GROUP_DIR in "$BASE_DIR"/*/; do
  ...
  mvn exec:java -Dexec.mainClass="edu.northeastern.Main" \
                -Dexec.args="-f ${GROUP_DIR} -t ${ORACLE_FILE} -a -e incomplete-lib,spec-gen -r html" -q
done