...
public class ModifyStartDate implements ModifyEvent {
  ...
  public EventReadOnly edit(CalendarEditable calendar, EventReadOnly event, String newValue) {
    ...
    LocalDateTime eightAm = LocalDateTime
        .parse(newValue, formatter).toLocalDate().atTime(8, 0);
    ...
    if (newStartDateTime.isAfter(event.getEndDateTime())) {
      ...
      newEndDateTime = newStartDateTime.plus(duration);
    }

    EventReadOnly updatedEvent;
    boolean isAllDay = newStartDateTime.equals(eightAm) && newEndDateTime.equals(fivePm);
