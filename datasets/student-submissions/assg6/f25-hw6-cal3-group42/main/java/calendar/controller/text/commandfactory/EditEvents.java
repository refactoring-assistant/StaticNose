...
/**
 * Class to edit a parameter in events starting from some date.
 */
public class EditEvents implements CommandInterface {
   ...
   */
  public EditEvents(Map<String, Object> parameters,
    ...
    if (calendar == null || view == null) {
      throw new IllegalArgumentException(
          "Calendar or View is non-existent in EditEvents");
    }
  ...
  public void execute() {
    try {
      model.editEvents(this.parameters);
      String displayMessage = "Using calendar " + calendar.getCalendarName() + ": "
          + "Successfully edited all events called "
          + this.parameters.get("subject") + " starting from " + this.parameters.get("start")
          + " and changed " + this.parameters.get("property") + " to "
          + this.parameters.get("value") + ".";
      view.display(displayMessage);
    } catch (Exception e) {
      view.displayError("EditEvents command: " + e.getMessage());
    }
  }
}
