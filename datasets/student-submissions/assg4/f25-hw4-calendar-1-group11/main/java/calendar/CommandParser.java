...
public class CommandParser {
   ...
   * - create event "subject" on YYYY-MM-DD (all-day event)
   * - create event "subject" from ... repeats MWF for 10 times
   * - create event "subject" on ... repeats MWF for 10 times (all-day series).
   */
   ...
   * create event "subject" from YYYY-MM-DDTHH:mm to YYYY-MM-DDTHH:mm [at "location"]
   * [description "desc"].
   */
  private void parseCreateSingleEvent(String remainder) {
    int fromIndex = remainder.indexOf(" from ");
    if (fromIndex == -1) {
      throw new IllegalArgumentException("Missing 'from' in create event command");
    }
    ...
    if (toIndex == -1) {
      throw new IllegalArgumentException("Missing 'to' in create event command");
    }
   ...
   * create event "subject" on YYYY-MM-DD [at "location"] [description "desc"].
   */
  private void parseCreateAllDayEvent(String remainder) {
    int onIndex = remainder.indexOf(" on ");
    if (onIndex == -1) {
      throw new IllegalArgumentException("Missing 'on' in create event command");
    }
  ...
  private void parseCreateRepeatingEvent(String remainder) {
    ...
    if (repeatsIndex == -1) {
      throw new IllegalArgumentException("Missing 'repeats' in create event command");
    }
  ...
  private void parseEditSingleEvent(String trimmed) {
    ...
    if (firstSpace == -1) {
      throw new IllegalArgumentException("Invalid edit event syntax: missing property");
    }
    final String property = remainder.substring(0, firstSpace).trim();
    remainder = remainder.substring(firstSpace + 1).trim();

    int fromIndex = remainder.indexOf(" from ");
    if (fromIndex == -1) {
      throw new IllegalArgumentException("Invalid edit event syntax: missing 'from'");
    }
    ...
    int toIndex = afterFrom.indexOf(" to ");
    if (toIndex == -1) {
      throw new IllegalArgumentException("Invalid edit event syntax: missing 'to'");
    }
    ...
    if (withIndex == -1) {
      throw new IllegalArgumentException("Invalid edit event syntax: missing 'with'");
    }
  ...
  private void parseEditCommandWithoutTo(String trimmed, String prefix, EditType editType) {
    ...
    int firstSpace = remainder.indexOf(' ');
    if (firstSpace == -1) {
      throw new IllegalArgumentException("Invalid " + prefix + " syntax: missing property");
    }
    final String property = remainder.substring(0, firstSpace).trim();
    remainder = remainder.substring(firstSpace + 1).trim();
    ...
    int fromIndex = remainder.indexOf(" from ");
    if (fromIndex == -1) {
      throw new IllegalArgumentException("Invalid " + prefix + " syntax: missing 'from'");
    }
    ...
    if (withIndex == -1) {
      throw new IllegalArgumentException("Invalid " + prefix + " syntax: missing 'with'");
    }
  ...
  private void setOptionalProperties(String subject, LocalDateTime startDateTime,
    ...
    if (atIndex != -1) {
      String afterAt = remainingText.substring(atIndex + 4);
      int nextDescIndex = afterAt.indexOf(" description ");

      if (nextDescIndex != -1) {
        location = afterAt.substring(0, nextDescIndex).trim();
      } else {
        location = afterAt.trim();
      }
    ...
    if (descIndex != -1) {
      description = remainingText.substring(descIndex + 13).trim();
